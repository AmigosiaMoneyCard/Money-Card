...
function calculateBranchMetrics(unifiedProducts: ProductWithInventory[]): BranchMetrics {
  ...
  const totalValuation = unifiedProducts.reduce(
...
function useBranchDetailsData(branch: Branch | null, isOpen: boolean) {
  ...
  const branchMetrics = useMemo(
    () => calculateBranchMetrics(unifiedProducts),
    [unifiedProducts],
  );
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-5">
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-1">
        ...
        <p className="text-lg font-bold text-slate-900">{staffCount}</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-1">
        ...
        <p className="text-[10px] text-emerald-600 font-medium">
          {metrics.activeProducts} Active on sale
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-1 min-w-0">
        ...
        <p
          ...
          title={`${metrics.totalStockUnits.toLocaleString('en-IN')} units`}
        >
          {formatCompactNumber(metrics.totalStockUnits)}
        </p>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-1 min-w-0">
        ...
        <p
          className="text-lg font-bold text-teal-700 font-mono truncate"
          title={formatCurrency(metrics.totalValuation)}
        >
          {formatCompactCurrency(metrics.totalValuation)}
        </p>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-1 col-span-2 sm:col-span-1">
        ...
        {metrics.totalAlerts === 0 ? (
          ...
          <>
            <p className="text-lg font-bold text-amber-700">{metrics.totalAlerts} Alerts</p>
      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {filteredProducts.map((p) => {
            const isVeg =
              Array.isArray(p.category) && p.category.some((c) => c.toLowerCase() === 'veg');
            const isNonVeg =
              Array.isArray(p.category) && p.category.some((c) => c.toLowerCase() === 'non-veg');
            const isBeverage =
              Array.isArray(p.category) &&
              p.category.some((c) => c.toLowerCase() === 'beverage' || c.toLowerCase() === 'drink');
...
function BranchAlertsTab({
  ...
  branch: Branch;
  metrics: BranchMetrics;
  unifiedProducts: ProductWithInventory[];
  onClose: () => void;
  onOpenMenu?: (branch: Branch) => void;
}) {
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 flex items-start gap-2.5 text-xs text-amber-900">
        ...
        <div>
          ...
          <p className="mt-0.5 text-amber-800">
            {metrics.outOfStockCount} items out of stock and {metrics.lowStockCount} items running low. Update stock quantities to prevent counter disruption.
          </p>
        </div>
      </div>
          ...
          .map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`rounded-xl border p-3 flex items-center justify-between gap-3 ${
                  qty === 0 ? 'border-rose-200 bg-rose-50/50' : 'border-amber-200 bg-amber-50/50'
                }`}
              >
                <div className="min-w-0">
...
export function BranchDetailsModal({
  ...
  const {
    ...
    branchMetrics,
    filteredProducts,
  } = useBranchDetailsData(branch, isOpen);
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <BranchKpiCards staffCount={staffList.length} metrics={branchMetrics} />
        ...
        <div className="min-h-[260px] max-h-[46vh] overflow-y-auto pr-1">
          {isLoading ? (
            ...
            <>
              ...
              {activeTab === 'STAFF' && (
                <BranchStaffTab branch={branch} staffList={staffList} />
              )}
              {activeTab === 'ALERTS' && (
                <BranchAlertsTab
                  branch={branch}
              ...
              {activeTab === 'OVERVIEW' && (
                <BranchOverviewTab branch={branch} metrics={branchMetrics} />
              )}
            </>
          )}
        </div>
